...
releases/download/v0.1-weights/efficientnet_b0-d6904d92.pth"
    ),
...
releases/download/v0.1-weights/efficientnet_b1-533bc792.pth",
    ...
    "efficientnet_b2": _cfg(
        url="https://github.com/rwightman/pytorch-image-models/\
releases/download/v0.1-weights/efficientnet_b2-cf78dc4d.pth",
...
def _initialize_weight_goog(shape=None, layer_type="conv", bias=False):
    ...
    if layer_type == "conv":
        assert (
            isinstance(shape, (tuple, list)) and len(shape) == 3
        ), "The shape must be 3 scalars, and are in_chs, ks, out_chs respectively"
        n = shape[1] * shape[1] * shape[2]
        return Normal(math.sqrt(2.0 / n))
    if layer_type == "bn":
        return One()
    assert (
        isinstance(shape, (tuple, list)) and len(shape) == 2
    ), "The shape must be 2 scalars, and are in_chs, out_chs respectively"
    n = shape[1]
...
def _conv1x1(
    in_channels, out_channels, stride=1, padding=0, pad_mode="same", bias=False
):
...
def _dense(in_channels, output_channels, bias=True, activation=None):
    weight_init_value = _initialize_weight_goog(
        shape=(in_channels, output_channels), layer_type="fc"
    )
    bias_init_value = _initialize_weight_goog(bias=True) if bias else None
    if bias:
        return nn.Dense(
...
def _decode_block_str(block_str, depth_multiplier=1.0):
    ...
    act_fn = options.get("n") if "n" in options else None
    exp_kernel_size = _parse_ksize(options.get("a")) if "a" in options else 1
    pw_kernel_size = _parse_ksize(options.get("p")) if "p" in options else 1
    fake_in_chs = int(options.get("fc")) if "fc" in options else 0

    num_repeat = int(options.get("r"))
    # each type of block has different valid arguments, fill accordingly
    if block_type == "ir":
        block_args = dict(
            block_type=block_type,
            dw_kernel_size=_parse_ksize(options.get("k")),
            exp_kernel_size=exp_kernel_size,
            pw_kernel_size=pw_kernel_size,
            out_chs=int(options.get("c")),
            exp_ratio=float(options.get("e")),
            se_ratio=float(options.get("se")) if "se" in options else None,
        ...
        block_args = dict(
            block_type=block_type,
            exp_kernel_size=_parse_ksize(options.get("k")),
            pw_kernel_size=pw_kernel_size,
            out_chs=int(options.get("c")),
            exp_ratio=float(options.get("e")),
...
def _decode_arch_def(arch_def, depth_multiplier=1.0, depth_trunc="ceil"):
    ...
    for _, block_strings in enumerate(arch_def):
        ...
        for block_str in block_strings:
            ...
            repeats.append(rep)
        arch_args.append(
            _scale_stage_depth(stack_args, repeats, depth_multiplier, depth_trunc)
        )
    return arch_args
...
class BlockBuilder(nn.Cell):
    ...
    def _make_block(self, ba):
        ...
        if bt == "ir":
            ...
            if self.verbose:
                logging.info(
                    "  DepthwiseSeparable %d, Args: %s", self.block_idx, str(ba)
                )
...
class DropConnect(nn.Cell):
    def __init__(self, drop_connect_rate=0.0, seed0=0, seed1=0):
...
class GenEfficientNet(nn.Cell):
    def __init__(
        ...
        if not head_conv or head_conv == "none":
            ...
            self.bn2 = (
                None if self.efficient_head else _fused_bn(self.num_features, **bn_args)
            )
