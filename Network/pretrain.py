...
sys.path.append("/data1/home/guanzhiyuan/models/community/cv/RHFL/Dataset")
print(sys.path)
...
f = open("/data1/home/guanzhiyuan/models/community/cv/RHFL/config.yaml", "r", encoding="utf-8")
fdata = f.read()
f.close()
...
def evaluate_network(net, dataloader, loss_function):
    ...
    print(
        "Test Accuracy of the model on the test images: {} %".format(100 * acc["acc"])
    )
    return 100 * acc["acc"]
...
if __name__ == "__main__":
    ...
    for index in range(n_participants):
        ...
        logger.info(
            "Pretrain the "
            + str(index)
            + " th Participant Model with N_training: "
            + str(len(train_ds_local))
        )
