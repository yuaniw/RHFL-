    ...
    def __init__(
        ...
        if self.train:
            ...
            self.train_data = self.train_data.reshape(
                (50000, 3, 32, 32)
            )  # Convert the image data format to [height, width, channels]
            ...
            if noise_type != "clean":
                ...
                training_labels = [i[0] for i in self.train_labels]
...
class CIFAR100:
    ...
    def __init__(
        ...
        if self.train:
            ...
            self.train_noisy_labels = [i[0] for i in self.train_noisy_labels]
            training_labels = [i[0] for i in self.train_labels]
            self.noise_or_not = np.transpose(self.train_noisy_labels) == np.transpose(
                training_labels
            )
        else:
            f = self.test_list[0][0]
...
def noisify_pairflip(y_train, noise, random_state=None, nb_classes=10):
    ...
    flip in the pair
    """
    pair = np.eye(nb_classes)
    n = noise

    if n > 0.0:
        # 0 -> 1
        pair[0, 0], pair[0, 1] = 1.0 - n, n
        for i in range(1, nb_classes - 1):
            pair[i, i], pair[i, i + 1] = 1.0 - n, n
        pair[nb_classes - 1, nb_classes - 1], pair[nb_classes - 1, 0] = 1.0 - n, n

        y_train_noisy = multiclass_noisify(y_train, p=pair, random_state=random_state)
...
def noisify_multiclass_symmetric(y_train, noise, random_state=None, nb_classes=10):
    ...
    """
    pair = np.ones((nb_classes, nb_classes))
    n = noise
    pair = (n / (nb_classes - 1)) * pair
    ...
    if n > 0.0:
        ...
        for i in range(1, nb_classes - 1):
            pair[i, i] = 1.0 - n
        pair[nb_classes - 1, nb_classes - 1] = 1.0 - n
